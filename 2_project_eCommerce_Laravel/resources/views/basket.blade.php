    ...
    <div class="card-body">
        <div class="row">
			<div class="table-responsive">
					...
					@foreach($orders as $order)
				      ...
				      <td>
				       ...
				       <span class="count">{{$order->count}}</span>
				          ...
				          <form action="/basket/minus2/{{$order->id}}" method="POST">
				            {{ csrf_field() }}    
				            <button type="submit" class="btn btn-danger">-</button> 
				          </form> 

				          <form action="/basket/update2/{{$order->id}}" method="POST">
				            {{ csrf_field() }}  
				            <button type="submit" class="btn btn-primary">Обновить</button> 
				          </form>

				          <form action="/basket/add2/{{$order->id}}" method="POST">
				            {{ csrf_field() }}  
				      ...
				      <td><a href="/deleteBrand2/{{$order->id}}" class="btn btn-danger">x</a></td>
				    </tr>   
					@endforeach
				...
				<a class="btn btn-danger" href="/basket/destroy2/">Удалить все</a> 
                <a class="btn btn-success" href="/order/NoneAuth/">Оформить заказ</a>  
			</div>
    	</div>				
    </div>
</div>
