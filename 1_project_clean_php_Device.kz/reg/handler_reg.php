...
{ 
 ...
 include("../functions/functions.php");
    ...
    {
       $error[] = "Логин должен быть от 5 до 15 симвалов!"; 
    }
    else
    {   
    ...
    if (strlen($pass) < 7 or strlen($pass) > 15) $error[] = "Укажите пароль от 7 до 15 симвалов!";
    if (strlen($surname) < 3 or strlen($surname) > 20) $error[] = "Укажите фамилье от 3 до 20 симвалов!";
    if (strlen($name) < 3 or strlen($name) > 15) $error[] = "Укажите Имя от 3 до 15 симвалов!";
    if (strlen($patronymic) < 3 or strlen($patronymic) > 25) $error[] = "Укажите отчество от 3 до 25 симвалов!";
    ...
    if (!$address) $error[] = "Необходимо указывать адрес доставки!";
    
    if($_SESSION['img_captcha'] != strtolower($_POST['reg_captcha'])) $error[] = "Не верный код проверки!";
 ...
 echo implode('<br />',$error);
   ...
   {   
        ...
        mysql_query("   INSERT INTO reg_user(login,pass,surname,name,patronymic,email,phone,address,datetime,ip)/*вносим данные переменных в бд */
                        VALUES(
