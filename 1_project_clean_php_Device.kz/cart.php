...
{
if ( $_SESSION['auth'] == 'yes_auth' ) //если авториз дабовляем в таблицу ордерс
 {
...
$int = $row["cart_price"] * $row["cart_count"];//цену умножаем на колич
$all_price = $all_price + $int;//получаем общую сумму
...
$height = 105;
        ...
        <div id="tovar'.$row["cart_id"].'" class="price-product"><h5><span class="span-count" >'.$row["cart_count"].'</span> x <span>'.$row["cart_price"].'</span></h5><p price="'.$row["cart_price"].'" >'.group_numerals($int).' KZT</p></div>
 ...
 echo '
 <h2 class="itog-price" align="right">Итог: <strong>'.group_numerals($all_price).'</strong> KZT</h2>
...
<ul id="info-order">
';
  if ( $_SESSION['auth'] != 'yes_auth' ) //Если пользователь не авторизирован
{
echo '
...
echo '
<ul id="list-info" >
<li><strong>Способы доставки:</strong>'.$_SESSION['order_delivery'].'</li>
...
';    
 ...
 echo '
<h2 class="itog-price" align="right">Итого: <strong>'.group_numerals($itogpricecart).'</strong> KZT</h2>
...
$height = 105;
 ...
 echo '
    ...
    <h2 align="right" class="itog-price">Итог: <strong>'.group_numerals($all_price).'</strong> KZT</h2>
    <p align="right" class="button-next" ><a href="cart.php?action=confirm" >Далее</a></p> 
