...
do
{
$count = $count + $row["cart_count"];//к нему же прибавляем чтобы получить общее колич товаров 
$int = $int + ($row["price"] * $row["cart_count"]); //общая цена
}
...
if ($count > 81)
{
    $str=" тов";
     ...
     echo '<span>'.$count.$str.'</span> на сумму <span>'.group_numerals($int).'</span> KZT';
}
else
{
